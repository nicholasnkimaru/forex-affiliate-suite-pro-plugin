...
function fasp_pay_update($data) {
    ...
    // M-Pesa PEM certificate - use sanitize_textarea_field and validate PEM format
    $mpesa_cert_raw = isset($data['mpesa_cert']) ? sanitize_textarea_field($data['mpesa_cert']) : '';
    $clean['mpesa_cert'] = '';
    $clean['mpesa_cert_invalid'] = false;
    if (!empty($mpesa_cert_raw)) {
        ...
        if (strpos($mpesa_cert_raw, '-----BEGIN') !== false && strpos($mpesa_cert_raw, '-----END') !== false) {
            ...
            $clean['mpesa_cert_invalid'] = true;
            // Store an admin notice flag
            set_transient('fasp_mpesa_cert_invalid', true, 30);
        }
    }
...
function fasp_admin_payments_screen() {
    ...
    // Show admin notice for invalid PEM certificate
    if (get_transient('fasp_mpesa_cert_invalid')) {
        delete_transient('fasp_mpesa_cert_invalid');
        echo '<div class="notice notice-error"><p><strong>M-Pesa Certificate Error:</strong> The PEM certificate is invalid. It must contain valid BEGIN and END markers (e.g., -----BEGIN CERTIFICATE----- and -----END CERTIFICATE-----).</p></div>';
    }
