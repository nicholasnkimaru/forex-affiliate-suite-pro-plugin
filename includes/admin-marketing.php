...
function fasp_marketing_page(){
  ...
  if (isset($_POST['fasp_mkt_save']) && check_admin_referer('fasp_mkt','fasp_mkt_nonce')){
    ...
    // Save secrets option (non-autoload)
    update_option('fasp_marketing_secrets', $secrets);

    // Best-effort: mark non-autoload in DB
    global $wpdb;
  ...
  if (isset($_POST['fasp_mkt_replay']) && check_admin_referer('fasp_mkt','fasp_mkt_nonce')){
    ...
    $sent = 0;
    foreach ($dead as $i => $evt) { do_action('fasp/event/replay', $evt); $sent++; }
  ...
  <div class="wrap fasp-admin">
    ...
    <div class="fasp-wrap fasp-card">
      <form method="post"><?php wp_nonce_field('fasp_mkt','fasp_mkt_nonce'); ?>
        ...
        <p>
          <label>
            <input type="checkbox" name="enable_server_events" value="1" <?php checked($o['enable_server_events'],'1'); ?>>
            Enable server events (Meta CAPI + GA4 MP)
          </label>
        </p>
        <p>
          <label>
        ...
        <p>
          <label>Alert if last
            <input style="width:60px" name="alert_window_hours" value="<?php echo esc_attr($o['alert_window_hours']); ?>"> hours conversion % &lt;
            <input style="width:60px" name="alert_threshold" value="<?php echo esc_attr($o['alert_threshold']); ?>">%
          </label>
        </p>
        ...
        <p>
          <label>
            <input type="checkbox" name="google_ads_enable" value="1" <?php checked($o['google_ads_enable'],'1'); ?>>
            Enable Google Ads server conversions (placeholder)
          </label>
        </p>
        ...
        <p>
          <label>
            <input type="checkbox" name="tiktok_enable" value="1" <?php checked($o['tiktok_enable'],'1'); ?>>
            Enable TikTok Events API (placeholder)
          </label>
        </p>
      ...
      <?php if ($dead): ?>
        ...
        <?php foreach(array_slice(array_reverse($dead),0,50) as $e): ?>
          <tr>
            ...
            <td><?php echo esc_html($e['provider'] ?? ''); ?></td>
